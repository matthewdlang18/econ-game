...
async function startSinglePlayerGame() {
  try {
    ...
    // For single player mode, we need different approaches for TAs and students
    if (currentUser.role === 'ta' && currentUser.section_id) {
      try {
        // TAs can create game sessions according to the RLS policy
        const { data, error } = await window.SupabaseIntegration.createGameSession(currentUser.section_id);

        if (error) {
          console.error('Error creating game session:', error);
          // Fall back to special format ID
          gameId = 'single-player-' + currentUser.id;
      ...
      // For regular students, use a special format ID that works with RLS
      console.log('Using special format ID for single player game');

      // Use a consistent ID format for this user's single player games
      // This allows saving to the database without creating a game session
      gameId = 'single-player-' + currentUser.id;
