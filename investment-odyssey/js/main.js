...
async function startSinglePlayerGame() {
  try {
    ...
    // For single player mode, we'll use local storage by default
    // Only TAs can create game sessions according to the RLS policy
    if (currentUser.role === 'ta' && currentUser.section_id) {
      try {
        // Create a game session in the database (only works for TAs)
        const { data, error } = await window.SupabaseIntegration.createGameSession(currentUser.section_id);
      ...
      // For regular students, just use local storage
      console.log('Using local storage for single player game (not a TA)');
      gameId = 'local-' + Date.now();

      // Store section ID if available for potential future use
      if (currentUser.section_id) {
        sectionId = currentUser.section_id;
      }

      // Show notification about local storage
      window.UIController.showNotification('Game progress will be saved locally in your browser.', 'info');
    }
