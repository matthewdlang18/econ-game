...
async function startSinglePlayerGame() {
  try {
    ...
    // For single player mode, we need to create a proper game session in the database
    try {
      // Use a special section ID for single player games
      // We need to use the user's actual section_id if available, or create a special one
      const sectionId = currentUser.section_id || null;

      if (!sectionId) {
        console.warn('No section ID available for user, single player game state will be saved locally only');
        // Generate a client-side ID that won't be used for database operations
        gameId = 'local-' + Date.now();
      } else {
        // Create a game session in the database
        const { data, error } = await window.SupabaseIntegration.createGameSession(sectionId);
        ...
        if (error) {
          ...
          gameId = data.id;
          console.log('Created single player game session with ID:', gameId);
        }
      }
    } catch (error) {
      console.error('Error setting up game ID:', error);
      // Fall back to local ID
      gameId = 'local-' + Date.now();
    }
...
async function loadLeaderboard() {
  try {
    ...
    // If we have no online data or using local storage, merge with local entries
    if (leaderboardData.length === 0 || gameId.startsWith('local-')) {
      try {
...
async function handleSubmitScore() {
  ...
  try {
    ...
    // Only submit to database if we have a valid UUID game ID
    if (gameId && !gameId.startsWith('local-')) {
      ...
      console.log('Using local storage for leaderboard entry (no valid game ID)');
